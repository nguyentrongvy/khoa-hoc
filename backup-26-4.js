...
async function handleDownloadVideo1(_data, headerPhotoUpload, headers) {
...
async function handleDownloadVideo(_dbs, headerPhotoUpload, headers) {
...
async function run(headerPhotoUpload, headers, csvFile, _path_date_video = './video/17-4/', _path_date_json = './json/17-4/', _count) {
    ...
    //     let _pathFileJson = _line.fileJson;
    //     let _ctxJson = await getCtxJsonFile(_pathFileJson);
    //     let _json = JSON.parse(_ctxJson) || {};

    //     await handleDownloadVideo1(_data, headerPhotoUpload, headers);
    ...
    for (const _line of data) {
        ...
        _line.upload = _json.upload || false;
        _line.dowload = _json.dowload || false;

        if (_line && _line.id && !_json.upload && !_json.dowload) {

            if (isVideo(_line.type)) {
                const _result = await getLinkVideo(_line.id, _line.name, headers);
    ...
    await createFolderDataContain(_dbs, _path_date_video, _path_date_json);

    await handleDownloadVideo(_dbs, headerPhotoUpload, headers);
}
